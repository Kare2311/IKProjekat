...
#include <string.h>
#include <process.h>
#include "AdvancedHeapManager.cpp"
...
void handleClient(void* clientData) {
	...
	AdvancedHeapManager advancedHeapManager(5, 1024 * 1024);
		...
		if (iResult > 0) {
			...
			if (sscanf_s(recvBuffer, "%[^|]|%s", optionstr, sizeof(optionstr), message, sizeof(message)) == 2)
			{
				...
				case 1: {
					...
					void* allocatedMemory = advancedHeapManager.allocate(numBytes, clientIndex);

					// Send response to the client based on allocation success or failure
					if (allocatedMemory != nullptr) {
						// Memory allocated successfully
						sprintf_s(sendBuffer, "Memory allocation successful. Address: %p", allocatedMemory);
						printf("Memory allocation successful. Address:");
					}
					else {
						// Allocation failed
						sprintf_s(sendBuffer, "Memory allocation failed. Not enough space.");
				...
				case 2: {
					...
					break;
				case 3: {
					std::string memoryState = advancedHeapManager.getMemoryState();
					send(clientSocket, memoryState.c_str(), memoryState.size(), 0);
					break;
				}
				}
				case 0: {
